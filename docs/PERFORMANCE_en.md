...
- Mesh count: the worker transfers every renderable mesh generated by OCCT; there is no hard triangle cap. Narrow screens use a coarser linear-deflection parameter, which does not guarantee a triangle count.
- Triangulation parameters: `linearDeflection=0.004` on wide stages and `0.007` below 520px; `angularDeflection=0.16`. The implementation lives in `src/js/model-viewer.ts` and `src/js/step-worker.ts`.
- DPR: capped at 2.
