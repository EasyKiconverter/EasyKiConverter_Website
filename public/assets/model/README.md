...
The current website loads the source directly in the browser with the pinned `occt-import-js` 0.0.23 OpenCascade/WASM runtime. `src/js/step-worker.ts` performs loading, triangulation and bounded mesh preparation in a Web Worker; `src/js/model-viewer.ts` receives the transferable vertex buffer and renders it through Three.js. The browser never needs a CAD application, and the original STEP file is not replaced or deleted. If WebGL, WASM, the Worker or the STEP asset fails, the page keeps its content and uses the styled visual fallback.

Runtime parameters are `linearUnit: millimeter`, `linearDeflectionType: bounding_box_ratio`, `linearDeflection: 0.012`, and `angularDeflection: 0.35`. The renderer bounds the expanded triangle stream to 75,000 triangles on desktop and 36,000 on narrow screens. Source SHA256: `4310cdcbb920625dfdea4baac3855ec2fce4b7281ce335762c10791f0da939ce`.
...
The website does not claim that the bounded WebGL triangulation is a lossless CAD render. This keeps the source/derived-asset boundary auditable.
